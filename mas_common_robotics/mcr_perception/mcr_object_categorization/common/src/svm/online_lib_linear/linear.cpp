...
#include "svm/online_lib_linear/linear.h"
#include "svm/online_lib_linear/tron.h"
...
static void onlinesolve_l2r_l1l2_svc(
    const problem *prob, double *w, double *alpha, double eps,
    double Cp, double Cn, int solver_type)
{
    int l = prob->l;
...
static void onlinetrain_one(const problem *prob, const parameter *param, double *w, double *alpha, double Cp, double Cn)
{
    double eps = param->eps;
    int pos = 0;
    int neg = 0;
    ...
    case L2R_L2LOSS_SVC_DUAL:
        onlinesolve_l2r_l1l2_svc(prob, w, alpha, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL);
        break;
    case L2R_L1LOSS_SVC_DUAL:
        onlinesolve_l2r_l1l2_svc(prob, w, alpha, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL);
        break;
    case L1R_L2LOSS_SVC:
    {
        problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
    ...
    int nr_class;
    int nr_sample = model_->nr_sample;
    int *label = NULL;
    int *start = NULL;
    int *count = NULL;
    ...
    if (param->solver_type == MCSVM_CS)
    {
        ...
        model_->alpha = Malloc(double, model_->nr_sample * nr_class);
        for (i = 0; i < nr_class; i++)
    ...
    else
    {
        if (nr_class == 2)
        {
            ...
            model_->alpha = Malloc(double, nr_sample);
            for (i = 0; i < l; i++)
                model_->alpha[i] = 0;
            ...
            train_one(&sub_prob, param, &model_->w[0], &model_->alpha[0], weighted_C[0], weighted_C[1]);
        }
        else
        {
            model_->w = Malloc(double, w_size * nr_class);
            model_->alpha = Malloc(double, nr_sample * nr_class);
            double *w = Malloc(double, w_size);
            double *alpha = Malloc(double, nr_sample);
            ...
            for (i = 0; i < nr_class; i++)
            {
                ...
                for (int j = 0; j < nr_sample; j++)
                    model_->alpha[perm[j]*nr_class + i] = alpha[j];
            }
            free(w);
            free(alpha);
        }
...
// Interface functions
//
void onlinetrain(const problem *prob, const parameter *param, struct model *model_, const bool *update)
{
    int i, j;
    int l = prob->l;
    int n = prob->n;
    int w_size = prob->n;
    int nr_sample_old = model_->nr_sample;
    model_->nr_sample = l;
//  model *model_ = Malloc(model,1);
//
//  if(prob->bias>=0)
...
//          model_->w=Malloc(double, w_size);
            ...
            onlinetrain_one(&sub_prob, param, &model_->w[0], &model_->alpha[0], weighted_C[0], weighted_C[1]);
        }
        else
        {
            double *w = Malloc(double, w_size);
            double *modelw = Malloc(double, w_size * nr_class_old);
            double *alpha_c = Malloc(double, l);
            double *alpha = Malloc(double, l);
            double *modelalpha = Malloc(double, nr_sample_old * nr_class_old);
            for (i = 0; i < nr_class_old; i++)
            {
                for (int j = 0; j < w_size; j++)
                    modelw[j * nr_class_old + i] = model_->w[j * nr_class_old + i];
                for (int j = 0; j < nr_sample_old; j++)
                    modelalpha[j * nr_class_old + i] = model_->alpha[j * nr_class_old + i];
            }
            free(model_->w);
            model_->w = Malloc(double, w_size * nr_class);
            free(model_->alpha);
            ...
            for (i = 0; i < nr_class; i++)
            {
                ...
                {
                    for (int j = 0; j < w_size; j++)
                        w[j] = modelw[j * nr_class_old + i];
                    for (int j = 0; j < nr_sample_old; j++)
                    {
                        // Figure out position
                        alpha_c[j] = modelalpha[j * nr_class_old + i];
                    }

                    for (int j = 0; j < l; j++)
                    {
                        alpha[j] = alpha_c[perm[j]];
                    }

                }
                if (i >= nr_class_old || update[i])
                    onlinetrain_one(&sub_prob, param, w, alpha, weighted_C[i], param->C);
...
int save_model(const char *model_file_name, const struct model *model_)
{
    ...
    for (i = 0; i < nr_sample; i++)
    {
        int j;
        for (j = 0; j < nr_w; j++)
            fprintf(fp, "%.16g ", model_->alpha[i * nr_w + j]);
        fprintf(fp, "\n");
    }
...
struct model *load_model(const char *model_file_name)
{
    ...
    while (1)
    {
        ...
        else if (strcmp(cmd, "nr_sample") == 0)
        {
            fscanf(fp, "%d", &nr_sample);
            model_->nr_sample = nr_sample;
        }
    ...
    fscanf(fp, "%80s", cmd);
    model_->alpha = Malloc(double, nr_sample*nr_w);
    for (i = 0; i < nr_sample; i++)
    {
        int j;
        for (j = 0; j < nr_w; j++)
            fscanf(fp, "%lf ", &model_->alpha[i * nr_w + j]);
        fscanf(fp, "\n");
    }
...
int get_nr_sample(const model *model_)
{
    return model_->nr_sample;
}
