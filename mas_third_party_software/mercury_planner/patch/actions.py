    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, objects_by_type):
        ...
        if effects:
            if self.cost is None:
                ...
                cost = 0                                                                                # added lines for multiple cost increase
                for each in self.cost:                                                                  # added lines for multiple cost increase
                    cost = cost + int(each.instantiate(var_mapping, init_facts).expression.value)       # added lines for multiple cost increase
                #cost = int(self.cost.instantiate(var_mapping, init_facts).expression.value) #original line from downward!!
