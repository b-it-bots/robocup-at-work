...
def parse_effects(alist, result):
    ...
    cost_array = []                             # added lines for multiple cost increase
    for each in tmp_effect.effects:             # added lines for multiple cost increase
        if isinstance(each, CostEffect):        # added lines for multiple cost increase
            cost_array.append(each.effect)      # added lines for multiple cost increase
    normalized = tmp_effect.normalize()
    cost_eff, rest_effect = normalized.extract_cost()
    add_effect(rest_effect, result)
    if cost_eff:
        #return cost_eff.effect         # original downward line
        return cost_array               # added lines for multiple cost increase
    else:
        return None
